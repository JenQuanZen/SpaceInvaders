...
ANISPRITE ship;
ANISPRITE greenEnemies[8];
ANISPRITE yellowEnemies[8];
ANISPRITE blueEnemies[8];
ANISPRITE yelEnemies[8];
ANISPRITE greEnemies[8];
ANISPRITE shipBullets[10];
ANISPRITE enemyBullets[10];
...
void initializeGame() {
 ...
 DMANow(3, &spriteSheetTiles, &((charblock *)0x6000000)[4], 16384);
...
void drawGame() {
    ...
    DMANow(3, gameBGTiles, &((charblock *)0x6000000)[0], 16);
    DMANow(3, gameBGMap, &((screenblock *)0x6000000)[31], 1024);


    shadowOAM[0].attr0 = (0<<8) | (0<<13) | (0<<14) | ship.row;
    shadowOAM[0].attr1 = (1<<14) | ship.col;
    shadowOAM[0].attr2 = ((2 * ship.curFrame)*32+(2 * ship.aniState)) | ((0)<<12);



    for (int i = 0; i < 8; i++) {
        if (greenEnemies[i].active) {
            shadowOAM[1 + i].attr0 = (0<<8) | (0<<13) | (0<<14) | greenEnemies[i].row;
            shadowOAM[1 + i].attr1 = (1<<14) | greenEnemies[i].col;
            shadowOAM[1 + i].attr2 = ((2 * greenEnemies[i].curFrame)*32+(2 * greenEnemies[i].aniState)) | ((0)<<12);
        } else {
    ...
    for (int i = 0; i < 8; i++) {
        if (yellowEnemies[i].active) {
            shadowOAM[1 + 8 + i].attr0 = (0<<8) | (0<<13) | (0<<14) | yellowEnemies[i].row;
            shadowOAM[1 + 8 + i].attr1 = (1<<14) | yellowEnemies[i].col;
            shadowOAM[1 + 8 + i].attr2 = ((2 * yellowEnemies[i].curFrame)*32+(2 * yellowEnemies[i].aniState)) | ((0)<<12);
        } else {
            shadowOAM[1 + 8 + i].attr0 = (2<<8);
        }
    }

    for (int i = 0; i < 8; i++) {
        if (blueEnemies[i].active) {
            shadowOAM[1 + 8 * 2 + i].attr0 = (0<<8) | (0<<13) | (0<<14) | blueEnemies[i].row;
            shadowOAM[1 + 8 * 2 + i].attr1 = (1<<14) | blueEnemies[i].col;
            shadowOAM[1 + 8 * 2 + i].attr2 = ((2 * blueEnemies[i].curFrame)*32+(2 * blueEnemies[i].aniState)) | ((0)<<12);
        } else {
            shadowOAM[1 + 8 * 2 + i].attr0 = (2<<8);
        }
    }

    for (int i = 0; i < 8; i++) {
        if (yelEnemies[i].active) {
            shadowOAM[1 + 8 * 3 + i].attr0 = (0<<8) | (0<<13) | (0<<14) | yelEnemies[i].row;
            shadowOAM[1 + 8 * 3 + i].attr1 = (1<<14) | yelEnemies[i].col;
            shadowOAM[1 + 8 * 3 + i].attr2 = ((2 * yelEnemies[i].curFrame)*32+(2 * yelEnemies[i].aniState)) | ((0)<<12);
        } else {
            shadowOAM[1 + 8 * 3 + i].attr0 = (2<<8);
        }
    }

    for (int i = 0; i < 8; i++) {
        if (greEnemies[i].active) {
            shadowOAM[1 + 8 * 4 + i].attr0 = (0<<8) | (0<<13) | (0<<14) | greEnemies[i].row;
            shadowOAM[1 + 8 * 4 + i].attr1 = (1<<14) | greEnemies[i].col;
            shadowOAM[1 + 8 * 4 + i].attr2 = ((2 * greEnemies[i].curFrame)*32+(2 * greEnemies[i].aniState)) | ((0)<<12);
        } else {
            shadowOAM[1 + 8 * 4 + i].attr0 = (2<<8);
        }
    }
    ...
    for (int i = 0; i < 10; i++) {
        if (shipBullets[i].active) {
            shadowOAM[1 + 8 * 5 + i].attr0 = (0<<8) | (0<<13) | (2<<14) | shipBullets[i].row;
            shadowOAM[1 + 8 * 5 + i].attr1 = (0<<14) | shipBullets[i].col;
            shadowOAM[1 + 8 * 5 + i].attr2 = ((2 * shipBullets[i].curFrame)*32+(shipBullets[i].aniState)) | ((0)<<12);
        } else {
            shadowOAM[1 + 8 * 5 + i].attr0 = (2<<8);
        }
    }

    for (int i = 0; i < 10; i++) {
        if (enemyBullets[i].active) {
            shadowOAM[1 + 8 * 5 + 10 + i].attr0 = (0<<8) | (0<<13) | (2<<14) | enemyBullets[i].row;
            shadowOAM[1 + 8 * 5 + 10 + i].attr1 = (0<<14) | enemyBullets[i].col;
            shadowOAM[1 + 8 * 5 + 10 + i].attr2 = ((2 * enemyBullets[i].curFrame)*32+(enemyBullets[i].aniState)) | ((0)<<12);
        } else {
            shadowOAM[1 + 8 * 5 + 10 + i].attr0 = (2<<8);
        }
    }
...
void updateShip() {
    ...
    if ((!(~(oldButtons)&((1<<0))) && (~buttons & ((1<<0)))) && bulletTimer >= 30) {
        fireBullets();
        bulletTimer = 0;
    }
